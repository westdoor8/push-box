...
void mainloop() {
	...
	//绘制界面
	//主界面
	char *main_interface[] = {//这种定义方式更省内存
		"\n\n\n\n\n\n\n",
		"\t\t\t\t\t\t\t 推箱子",
		"\t\t\t\t\t\t   |===============|",
		"\t\t\t\t\t\t   |               |",
...
int gameloop(int level)
{
	...
	while (1) {
		...
		case 1:
			...
			{//进入到关卡选择界面，但中途推出关卡选择的话不会继续当前游戏，会回到主菜单
				reValue = 2;
				isEnd = 1;
			}
			else if (option == 'q') {
...
void LoadMap(int level, char(*map)[50])
{
	...
	sprintf(buffer, "Debug\\Map\\%d.txt", level + 1);//通过sprintf函数格式化文件路径为我们要读取的文件
